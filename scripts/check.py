...
README_MARKERS = ("vim.cmd.colorscheme('apollo')", "vim.cmd.colorscheme('apollo-light')")
...
def _activate_guidance(markdown: str) -> str:
    section = re.search(r"(?ms)^## Activate[ \t]*\r?$\n(.*?)(?=^## |\Z)", markdown)
    return section.group(1) if section else ""


def _has_native_command(guidance: str, marker: str) -> bool:
    command = re.escape(marker)
    standalone = re.search(rf"(?m)^[ \t]*{command}[ \t]*\r?$", guidance)
    inline = re.search(
        rf"(?P<ticks>`+)(?!`)[ \t]*{command}[ \t]*(?P=ticks)(?!`)",
        guidance,
    )
    return standalone is not None or inline is not None
...
def validate_readme_contract(markdown: str) -> None:
    ...
    guidance = _activate_guidance(markdown)
    for marker in README_MARKERS:
        if not _has_native_command(guidance, marker):
            raise AssertionError(f"README Activate guidance must include native marker {marker}")
